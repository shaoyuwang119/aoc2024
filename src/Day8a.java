		...
		for (ArrayList<int[]> antsList : freqs.values()) {
		    for (int[] pos : antsList) {
		        for (int[] testPos : antsList) {
		            ...
		            if (pos[0]-dist[0] >= 0 && pos[0]-dist[0] < grid.size() && pos[1]-dist[1] >= 0 && pos[1]-dist[1] < grid.size()) {
		                System.out.println(Arrays.toString(pos) + " " + Arrays.toString(dist));
		                grid.get(pos[0]-dist[0]).set(pos[1]-dist[1], "#");
		            }
